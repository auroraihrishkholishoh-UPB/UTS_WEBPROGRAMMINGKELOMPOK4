              ...
              <tbody>
                ...
                <tr>
                  ...
                  <?php if ($row['status_bus'] == '1') { ?>
                    ...
                    <?php } else { ?>
                    <td class="btn-danger">Tidak Aktif</td>
                  <?php } ?>
                  <td align="center"><a href="<?= base_url('backend/bus/viewbus/'.$row['kd_bus'])?>" class="btn btn btn-info">Lihat</a>
                    <a href="<?= base_url('backend/bus/deletebus/'.$row['kd_bus'])?>" class="btn btn btn-danger" onclick="return confirm('Apakah Anda yakin menghapus bus ini?')">Hapus</a>
                  </td>
              </tr>
              <?php } ?>
            </tbody>
...
<div class="modal-content">
  ...
  <div class="modal-body">
    <form action="<?= base_url()?>backend/bus/tambahbus" method="post">
      ...
      <div class="form-group">
        <label for="platbus" class="">Plat Nomor Bus</label>
        <input type="text" class="form-control" name="plat_bus" placeholder="Plat Bus">
      </div>
      <div class="form-group">
        <label for="seat" class="">Jumlah Kursi (Maks.23)</label>
        <input type="number" class="form-control" id="seat" name="seat" placeholder="[Maksimum 23]">
      </div>
