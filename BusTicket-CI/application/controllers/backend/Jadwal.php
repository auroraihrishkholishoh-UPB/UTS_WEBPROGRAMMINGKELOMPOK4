...
class Jadwal extends CI_Controller {
	...
	public function deletejadwal($id=''){
		// Check if jadwal is used in order
		$cek = $this->db->query("SELECT * FROM tbl_order WHERE kd_jadwal = '".$id."'")->num_rows();
		if ($cek > 0) {
			$this->session->set_flashdata('message', 'swal("Failed", "Schedule is still used in order, cannot delete", "error");');
		} else {
			$this->db->where('kd_jadwal', $id);
			$this->db->delete('tbl_jadwal');
			$this->session->set_flashdata('message', 'swal("Succeed", "Schedule Data Deleted", "success");');
		}
		redirect('backend/jadwal');
	}
}
