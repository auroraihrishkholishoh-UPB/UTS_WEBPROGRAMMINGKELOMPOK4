...
class Bus extends CI_Controller {
	...
	public function deletebus($id=''){
		// Check if bus is used in jadwal
		$cek = $this->db->query("SELECT * FROM tbl_jadwal WHERE kd_bus = '".$id."'")->num_rows();
		if ($cek > 0) {
			$this->session->set_flashdata('message', 'swal("Failed", "Bus is still used in schedule, cannot delete", "error");');
		} else {
			$this->db->where('kd_bus', $id);
			$this->db->delete('tbl_bus');
			$this->session->set_flashdata('message', 'swal("Succeed", "Bus Data Deleted", "success");');
		}
		redirect('backend/bus');
	}
