...
class Tiket extends CI_Controller {
	...
	public function cekorder($id=''){
		...
		if ($sqlcek) {
			...
			// Check status of first ticket (all should have same status for same order code)
			$status = $sqlcek[0]['status_order'];
			
			// Status: 0=Cancel, 1=Pending, 2=Paid, 3=Expired
			if ($status == 2) {
				// Ticket already paid
				$this->load->view('frontend/payment_success', $data);
			} else if ($status == 3) {
				// Ticket expired
				$this->session->set_flashdata('message', 'swal("Expired", "Tiket Sudah Expired", "error");');
				redirect('tiket/cektiket');
			} else if ($status == 0) {
				// Ticket cancelled
				$this->session->set_flashdata('message', 'swal("Cancelled", "Tiket Sudah Dibatalkan", "error");');
				redirect('tiket/cektiket');
			} else {
				// Ticket pending - show payment form
				$this->load->view('frontend/payment', $data);
			}
		}else{
			$this->session->set_flashdata('message', 'swal("Empty", "No Tickets Found", "error");');
    		redirect('tiket/cektiket');
		}
	}
	...
	public function checkout($value=''){
		...
		//email
		$subject = 'BTBS - Booking Confirmation';
		$message = $this->load->view('frontend/sendmail',$send, TRUE);
		$to 	 = $this->session->userdata('email');
		
		// Load email config from application/config/email.php
		$this->load->library('email');
		$this->email->from('BTBS@busticket.com', 'Bus Ticket Booking System');
		$this->email->to($to);
		...
		if ($this->email->send()) {
			...
			// Email sending failed, but still show success message to user
			// Error details: $this->email->print_debugger()
			$this->session->set_flashdata('message', 'swal("Success", "Please proceed towards payment confirmation!", "success");');
			$this->load->view('frontend/checkout', $data);
		}
	}
	...
	public function cancel($kd_order=''){
		$this->getsecurity();
		// Cek apakah tiket milik user dan status pending atau sudah dibayar
		$cek = $this->db->query("SELECT * FROM tbl_order WHERE kd_order = '".$kd_order."' AND kd_pelanggan = '".$this->session->userdata('kd_pelanggan')."' AND status_order IN ('1','2')")->row_array();
		if ($cek) {
			// Update status ke cancelled (3)
			$this->db->where('kd_order', $kd_order);
			$this->db->update('tbl_order', array('status_order' => '3'));
			
			// Kirim email notifikasi
			$send['sendmail'] = $this->db->query("SELECT * FROM tbl_order LEFT JOIN tbl_jadwal on tbl_order.kd_jadwal = tbl_jadwal.kd_jadwal LEFT JOIN tbl_tujuan on tbl_jadwal.kd_tujuan = tbl_tujuan.kd_tujuan LEFT JOIN tbl_bank on tbl_order.kd_bank = tbl_bank.kd_bank WHERE kd_order ='$kd_order'")->row_array();
			$send['count'] = count($send['sendmail']);
			$subject = 'BTBS - Tiket Dibatalkan';
			$message = $this->load->view('frontend/cancelmail',$send, TRUE);
			$to = $this->session->userdata('email');
			$config = [
				'mailtype'  => 'html',
				'charset'   => 'utf-8',
			...
			$this->session->set_flashdata('message', 'swal("Success", "Tiket berhasil dibatalkan", "success");');
		} else {
			$this->session->set_flashdata('message', 'swal("Error", "Tiket tidak dapat dibatalkan", "error");');
		}
		redirect('profile/tiketsaya/'.$this->session->userdata('kd_pelanggan'));
	}
